...
public class CaesarCipher{
    ...
    static void runEncryption(boolean encrypt){
        System.out.printf("\nBegin entering %smessage. Enter line \"%s\" to end message.\n",
        ...
        do{
            ...
            while(token.hasNext()) unfilteredSB.append(token.next());
            System.out.print("> ");
            line = sc.nextLine();
        ...
        if(encrypt){
            ...
            for(int i=1;i<26;i++){
                ...
                decryptShift(filteredStr, i, Math.min(snippetLength,filteredStr.length()));
                System.out.println();
            }
        }
        ...
        if(encrypt){
            System.out.println("\nCipher selected. Encrypted message:");
            encryptShift(filteredStr, cipher);
        } else {
            System.out.println("\nNumber selected is cipher. Decrypted message:");
    ...
    private static void encryptShift(String filteredStr, int cipher){
        for(int j=0; j<filteredStr.length(); j++){
            char c = (char)(filteredStr.charAt(j)+cipher);
            c -= (c>90)? 26:0;
            System.out.print(c);
